...
export function AIReviewPanel({
  ...
  const [value, setValue] = useState(finding.recommended_action);
  return (
    <Card className="overflow-hidden border-indigo-200">
      ...
      <div className="space-y-5 p-5">
        ...
        <div className="rounded-lg border border-teal-100 bg-teal-50/60 p-4">
          <p className="mb-1 flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-teal-800">
            <ShieldCheck size={14} />
            Recommended human action
          </p>
          {editing ? (
            <textarea
              className="mt-2 w-full rounded-lg border border-teal-300 bg-white p-3 text-sm"
              value={value}
            ...
            {editing ? (
              ...
              <>
                ...
                <Button
                  ...
                  variant="danger"
                  onClick={() =>
                    onAction("reject", "Not supported by reviewer assessment.")
                  }
                >
                  <X size={15} />
                  Reject
